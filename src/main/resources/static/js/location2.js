document.addEventListener("DOMContentLoaded", function() {
    ...
    // MySQL 데이터(주소) 값 가져오기
   fetch('/Shoots/location2/data')
       .then(response => response.json())
       .then(locations => {
           // locations 배열을 순회하면서 각 location에 대해 처리
           locations.forEach(location => {
               const address = location.address.split('Address: ')[1];

               getLatLngFromAddress(address, function(lat, lon) {
                   if (lat && lon) {
                       ...
                       console.log('위치 검색 실패: ' + location.address);
                   }
               });
           });
       })
